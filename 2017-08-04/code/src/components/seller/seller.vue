...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    data () {
      return {
        transitionShow: false, //自定义测试动画flag-与本项目无关；

        /*是否收藏 -- 从localStorage中读取*/
        //favorite:false
    ...
    methods: {
      ...
      _initPics(){
        if(this.seller.pics){
          ...
          let picList = document.getElementById('pic-wrapper');
          let _this = this;
          picList.width = width + 'px';
          this.$nextTick(function () {
            if(!_this.picScroll){
              _this.picScroll = new BScroll(picList,{ //图片的横向滚动
                scrollX:true,
