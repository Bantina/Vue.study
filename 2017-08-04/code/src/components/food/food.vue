...
<script type="text/ecmascript-6">
  ...
  export default {
      ...
      data(){
          return{
              showFlag:false,  //是否展示food
              //传入ratingSelect的数据依赖跟踪
              selectType:ALL,  //评价标签内容
              onlyContent:false, //是否只看有内容评价
              desc:{           //评价标签文本描述
                  all:'全部',
                  positive:'推荐',
                  negative:'吐槽'
      ...
      methods:{
          /*控制food页面的展示，在其父组件goods中用$refs调用*/
          show(){
             let _this = this;
             this.showFlag = true;
          ...
          /*是否 需要展示该条评论-主要和 只看有内容评价 的点击事件相关
          * @type 评价类型
          * @text 评价的文本内容
          * */
          needShow(type,text){
            if(this.onlyContent && !text){ //只显示内容且 //3.监听 props副本值改变，事件派发通知到父组件 没有内容
              return false;
            }
            //判断 标签类型
          ...
          /*调用从子组件ratingSelect $.emit()派发过来的方法。*/
          onSelectType(val){
              this.selectType = val;
          },
          onOnlyContent(val){
