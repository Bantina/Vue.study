...
<script type="text/ecmascript-6">
  ...
  export default {
      ...
      data(){
        return{
          //1.定义父组件传值接收到的props的副本 --给父组件派发事件做准备
          myselType:this.selectType,     //评价标签选择类型
          myonlyContent:this.onlyContent //是否只读取有内容评价
        }
      },
      computed:{
          /*计算 评价类型为 positive 的个数*/
          positives(){
      ...
      watch:{
          /*2.监听 父组件props的值的改变，子组件中 props副本值 改变*/
          selectType(val){
              this.myselType = val;
          },
          /*3.监听 props副本值改变，事件派发通知到父组件*/
          myselType(val){
