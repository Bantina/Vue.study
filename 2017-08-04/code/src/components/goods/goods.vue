...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    data(){
      return{
          goods:[],        //所有请求到的商品数据
          listHeight:[],   //存放每个标题索引对应的高度 数组；
          scrollY:0,       //右侧内容区域滚动的当前y轴高度
          selectedFood:{}  //被选中的商品
      }
    },
    computed:{
      ...
      currentIndex(){
        for(let i=0;i<this.listHeight.length;i++){
            ...
            // 当遍历到最后一个i时，i+1超出数组索引范围，height2=undefined, 所以 应首先判断 !height2
    ...
    methods:{
        ...
        _initScroll(){
          ...
          _this.foodsScroll = new BScroll(document.getElementById('foods-wrapper'),{
              probeType: 3, //实时监控滚动高度的 探针属性
              click:true
          });
          _this.foodsScroll.on('scroll',(pos)=>{ //监听scroll事件获取 滚动位置
            _this.scrollY = Math.abs(Math.round(pos.y));//先取整再取绝对值-为正；
          })

        },

        /* 计算右侧内容栏的滚动高度-每一栏起始高度的数组；*/
        _calculateHeight(){
          let foodList = document.getElementsByClassName('food-list-hook'); //li[]，每个li为标题及其之下相关的所有food的高度
          let height = 0;
